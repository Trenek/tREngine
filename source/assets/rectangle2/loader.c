...
void rec2LoadModel(const char *, struct ModelInput *model, struct GraphicsSetup *) {
    ...
    model->mesh = calloc(model->meshQuantity, sizeof(struct MeshInput));

    model->mesh->sizeOfVertex = sizeof(struct Rec2Vertex);
    ...
    for (size_t i = 0; i < 4; i += 1) {
        glm_vec3_dup(pos[i], REC2(model->mesh->vertices)[i].pos);
        glm_vec2_copy(tex[i], REC2(model->mesh->vertices)[i].tex);
        glm_vec4_dup(color[i], REC2(model->mesh->vertices)[i].color);
    }

    model->mesh->indices[0] = 0;
    model->mesh->indices[1] = 1;
    model->mesh->indices[2] = 3;
    model->mesh->indices[3] = 0;
    model->mesh->indices[4] = 2;
    model->mesh->indices[5] = 1;
}
